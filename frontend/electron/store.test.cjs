...
test('scoped account saves remain physically separate', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'eov-scoped-store-'));
  const store = createStore(directory);
  store.write('world--user-a', { tick: 2 });
  store.write('world--user-b', { tick: 9 });
  assert.deepEqual(store.read('world--user-a').payload, { tick: 2 });
  assert.deepEqual(store.read('world--user-b').payload, { tick: 9 });
  assert.deepEqual(Object.keys(store.diagnostics(['world--user-a']).namespaces), ['world--user-a']);
});
