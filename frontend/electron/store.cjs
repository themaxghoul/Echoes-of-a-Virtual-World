...
const ALLOWED_NAMESPACE = /^(world|jarvis|ledger)(--[a-zA-Z0-9_-]{3,128})?$/;
...
function createStore(userDataPath) {
  ...
  const pathsFor = (namespace) => {
    if (!ALLOWED_NAMESPACE.test(namespace)) throw new Error('Unsupported save namespace');
    return {
  ...
  function diagnostics(allowedNamespaces = null) {
    const result = { schemaVersion: SCHEMA_VERSION, saveDirectory: saveDir, namespaces: {} };
    const discovered = fs.readdirSync(saveDir)
      .filter((name) => name.endsWith('.json') && !name.endsWith('.backup.json'))
      .map((name) => name.slice(0, -5))
      .filter((name) => ALLOWED_NAMESPACE.test(name));
    const namespaces = Array.isArray(allowedNamespaces) ? allowedNamespaces.filter((name) => ALLOWED_NAMESPACE.test(name)) : discovered;
