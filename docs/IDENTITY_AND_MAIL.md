...
- **Character:** an in-world identity; characters are assigned to accounts rather than used as passwords or authentication authorities.
...
The alpha owner is configured as `Luciferous`, but the name is not authority. Desktop and server bootstrap bind ownership to the account UUID after private provisioning. Subsequent authorization requires that UUID, the owner flag, and the private owner role to agree. A rename preserves ownership; registering or copying the display name does not grant it. The server stores the binding in `system_settings.owner_identity`; `EOV_OWNER_USER_ID` may identify an existing account only before that binding exists and cannot override a conflicting persisted binding. On desktop, an existing Luciferous account from an older alpha can establish the missing one-time UUID binding by successfully authenticating with its own stored password. An incorrect password creates no binding, and new registrations cannot claim the reserved owner name without private provisioning.
