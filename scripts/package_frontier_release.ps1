...
$stage = Join-Path $repo '.release-stage-alpha34'
$clientArchive = Join-Path $outputs 'EoV-Alpha34-Windows-x64.zip'
$serverArchive = Join-Path $outputs 'EoV-Alpha34-Persistent-Server.zip'
$checksumFile = Join-Path $outputs 'EoV-Alpha34-SHA256.txt'
...
try {
    ...
    $clientStage = Join-Path $stage 'Echoes-of-Virtuality-Alpha34-Windows-x64'
    Copy-Item -LiteralPath $clientSource.FullName -Destination $clientStage -Recurse
    Copy-Item -LiteralPath (Join-Path $repo 'docs\ALPHA34_RELEASE_NOTES.md') -Destination (Join-Path $clientStage 'ALPHA34_RELEASE_NOTES.md')

    $serverStage = Join-Path $stage 'Echoes-of-Virtuality-Alpha34-Persistent-Server'
    ...
    foreach ($relative in @(
        ...
        'competency_engine.py', 'causal_ledger.py', 'auth_security.py', 'owner_policy.py', 'world_status.py',
    ...
    Copy-Item -LiteralPath (Join-Path $repo 'backend\economy') -Destination (Join-Path $serverStage 'economy') -Recurse
    ...
    Copy-Item -LiteralPath (Join-Path $repo 'backend\ALPHA34_WORLD_SERVER.env.example') -Destination (Join-Path $serverStage 'ALPHA34_WORLD_SERVER.env.example')
    Copy-Item -LiteralPath (Join-Path $repo 'docs\ALPHA34_RELEASE_NOTES.md') -Destination (Join-Path $serverStage 'ALPHA34_RELEASE_NOTES.md')
